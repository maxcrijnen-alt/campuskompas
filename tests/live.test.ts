...
it.skipIf(
...
)('live Supabase permissions, real writes and private storage', async () => {
  const db = publicDb();
  const { data: floor, error } = await db.from('floors').select('id');
  expect(error).toBeNull();
  expect(floor).toHaveLength(8);
  const { data: session, error: authError } = await db.auth.signInWithPassword({
  ...
  try {
    ...
    expect((await admin.from('first_year_tips').insert(row)).error).toBeNull();
    expect(
      (await publicDb().from('first_year_tips').select('id').eq('id', id)).data,
    ).toEqual([]);
    expect(
      (
        await admin
    ...
    const deniedNode = await publicDb()
      .from('route_nodes')
      .update({ accessible: true })
      .eq('id', 'R8-0-entry')
      .select('id');
    expect(deniedNode.error !== null || deniedNode.data?.length === 0).toBe(
      true,
    );
    const deniedGem = await publicDb()
      .from('hidden_gems')
